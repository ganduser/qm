...
\]
Se è \( d \) il numero di dimensioni, allora 
\begin{align*}
...
&= (E_{n_1} + \cdots + E_{n_d}) \psi_{n_1, n_2, \dots, n_d}(\vec{x}) \quad \implies
\end{align*}
\[
\implies \quad -\frac{\hbar^2}{2m} \frac{d^2}{d{x^{(i)}}^2} \psi_{n_i}(x^{(i)}) + V_i(x^{(i)})\psi_{n_i}(x^{(i)}) = E_{n_i} \psi_{n_i}(x^{(i)})
\]