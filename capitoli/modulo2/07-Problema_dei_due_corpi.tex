...
D'ora in poi studier\`o solo sistemi ad un corpo, infatti, nel sistema del centro di massa,
\[
\mathcal{H} = \frac{\vec{p}^2}{2m} + V(r)
\]
e tipicamente user\`o coordinate sferiche 
\[
\begin{cases}
...
\end{cases}
\]
per\`o adesso devo sepapare $ \vec{p} = -i\hbar \vec{\nabla} $. Per l'impostazione radiale
\[
...
Quindi ottengo che $ [\tilde{p}_r, r] = -i\hbar $. Noto che anche $ [\tilde{p}_r + f(r), r] = -i\hbar $. Il secondo termine \`e critico, quindi 
\[
...
Voglio sapere che cos'\`e $ \hat{\vec{p}}^2 $, partendo dal caso classico, per arrivare a quello quantistico. Uso l'identit\`a vettoriale, che nel caso classico \`e 
\[
(\vec{a} \mathbf{\cdot} \vec{b})^2 = |\vec{a}|^2 + |\vec{b}|^2 - |\vec{a} \wedge \vec{b}|^2 \quad (\text{da dimostrare}).
\]
Con ci\`o visto sopra
\[
...
\end{align*}
e ho ci\`o che cercavo. Poi sostituisco $ \vec{a} $ e $ \vec{b} $ con $ \vec{x} $ e $ \vec{p} $ e ho la relazione classica 
\[
...
\end{align*}
dove $ \vec{L} $ ha la propriet\`a che
\[
