...
\]
La \( \mathcal{H} \) è somma di hamiltoniane che commutano fra loro, quindi gli autostati della \( \mathcal{H} \) sono tutti gli autostati di \( \mathcal{H}_i \). Allora
\[
\mathcal{H} \ket{\psi_E} = E \ket{\psi_E} \quad \implies \quad E_{k_1, \dots, k_d} = E_{k_1} + \cdots + E_{k_d}.
\]
Inoltre,
\[
...
\]
Se il problema è separabile, ho questa utile condizione.
