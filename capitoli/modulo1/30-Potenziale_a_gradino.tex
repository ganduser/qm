...
\label{gradino}
\end{figure}
Lo schema è riportato in Figura \ref{gradino}.
\[
...
\]
Allora la \( \psi'' \) è discontinua perché lo è \( V \), ma quindi \( \psi \) e \( \psi' \) sono continue, così posso dividere il problema in due parti e poi imporre delle condizioni di raccordo.

\bigskip
\noindent
Per \( x < 0 \) ho \( \psi_1 \) tale che 
\[
\psi_1''(x) = -\frac{2m}{\hbar^2} E \psi_1(x) \; \implies \; \psi_1(x) = A e^{ikx} + B e^{-ikx}, \; k = \frac{\sqrt{2mE}}{\hbar}.
\]
Per \( x > 0 \)
\begin{align*}
\psi_2''(x) &= -\frac{2m}{\hbar^2} (E - V_0) \psi_2(x) \; \implies \\
&\; \implies \; \psi_2(x) = C e^{ik'x} + D e^{-ik'x}, \; k' = \sqrt{\frac{2m}{\hbar^2}(E - V_0)}
\end{align*}
e per ora impongo \( E > V_0 \).

\bigskip
\noindent
Poiché \( \psi_1(0) = \psi_2(0) \) e \( \psi_1'(0) = \psi_2'(0) \), allora mi restano due condizioni da imporre. Sfrutto la normalizzazione per la terza equazione e poi impongo \( D = 0 \). Allora 
\[
\begin{cases}
A + B = C \\
ik(A - B) = ik'C 
...
\end{cases}.
\]
Introduco la \textbf{corrente di probabilità}
\[
...
\]
\[
\frac{\partial \rho}{\partial t} > 0 \qquad \implies \qquad J(a) > J(b).
\]
...
&= J_A(x, t) + J_B(x, t).
\end{align*}
Poiché \( \ket{\psi} = \ket{k} \braket{k|\psi} + \ket{-k} \braket{-k|\psi} \), allora 
\[
\braket{J} = \frac{k}{m} \left( |\braket{k|\psi}|^2 - |\braket{-k|\psi}|^2 \right).
\]
Inoltre, essendo 
\[
\rho = |\psi|^2 \; \implies \; \frac{\partial \rho}{\partial t} = 0 \; \implies \; \frac{\partial J}{\partial x} = 0,
\]
quindi \( J \) è invariante per spostamento, quindi 
\[
J_A + J_B = J_C \text{ costante}
\]
e questa condizione deve valere nella prima e nella seconda regione su cui poi impongo le equazioni di continuità. Allora 
\[
...
\[
k' = \sqrt{\frac{2m}{\hbar}(E - V_0)} \quad \implies \quad k' = i \beta, \, \beta \in \mathbb{R}
\]
quindi \( \psi_2(x) = C e^{-\beta x} \), cioè l'andamento è esponenziale e la corrente \( J_C(x, t) = 0 \), quindi \( J_A = - J_B \) e 
\[
