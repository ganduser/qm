...
\begin{tikzpicture}
  \begin{scope}[rotate=270]
    \draw[color=blue, domain=-3:3]   plot (\x,{exp(-(\x+1)*(\x+1))})    node[right] {};
    \draw[color=red, domain=-3:3]   plot (\x,{exp(-(\x-1)*(\x-1))})    node[right] {};
  \end{scope}
  ...
  \draw[thick] (G) -- (H);
\end{tikzpicture}
\caption{Le due distribuzioni, in colori diversi, nel caso di granelli di sabbia in cui si lasci aperta una sola fenditura per volta. Sono piccate sulle fenditure.}
...
\begin{tikzpicture}
  \begin{scope}[rotate=270]
    \draw[color=red, domain=-3:3]   plot (\x,{exp(-(\x+1)*(\x+1)) + exp(-(\x-1)*(\x-1))})    node[right] {};
  \end{scope}
  ...
  \draw[thick] (K) -- (L);
\end{tikzpicture}
\caption{In rosso la distribuzione ottenuta con entrambe le fenditure aperte. $p(A \to C_k)$ rappresenta la probabilit\`a che un granello di sabbia arrivi sullo schermo in posizione $C_k$.}
...
\begin{tikzpicture}
  ...
  \begin{scope}[rotate=270, shift={(0,4)}]
    \draw[color=blue, domain=-3:3, samples=200, smooth]   plot (\x,{cos(10*\x r)*cos(10*\x r)*exp(-(\x*\x))}) node[right] {};
    \draw[color=red, domain=-3:3, samples=200, smooth]   plot (\x,{exp(-(\x*\x))}) node[right] {};
  \end{scope}
\end{tikzpicture}
