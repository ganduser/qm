...
\]
e questa è un'evoluzione temporale alla Schr\"odinger.
...
\end{align*}
ma in generale non siamo in grado di fare i calcoli. In questo caso c'è la formula di Baker-Campbell-Hausdorff. Se \( [A, B] \neq 0 \),
\[
...
\]
\[
\frac{d \hat{a}_H}{dt} = \frac{1}{i\hbar} [\hat{a}, \mathcal{H}] = -i\omega \hat{a} \quad \implies \quad \hat{a}_H(t) = e^{-i\omega t} \hat{a}_S
\]
\[
\frac{d \hat{a}_H^\dag}{dt} = \frac{1}{i\hbar} [\hat{a}^\dag, \mathcal{H}] = i\omega \hat{a}^\dag \quad \implies \quad \hat{a}_H^\dag(t) = e^{i\omega t} \hat{a}_S^\dag
\]
così disaccoppio \( \hat{x} \) e \( \hat{p} \) nelle equazioni differenziali ed ho lo stesso risultato.
