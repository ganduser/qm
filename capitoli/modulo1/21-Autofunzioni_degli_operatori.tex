...
\[
\braket{q|\hat{p}|k} = \hbar k \braket{q|k}, \quad \text{con } \braket{q|k} = \psi_k(q) \quad \implies
\]
\[
\implies \quad -i \hbar \frac{d}{dq} \psi_k(q) = \hbar k \psi_k(q) \quad \implies \quad \psi_k(q) = N_k e^{ikq}.
\]
Poiché
\[
...
&= 2 \pi |N_k|^2 \qquad \implies \qquad |N_k|^2 = \frac{1}{2\pi}
\end{align*}
\[
...
\end{align*}
\textbf{N.B.:} in generale, \( \tilde{\psi} \neq \psi \).
...
\]
\textbf{N.B.:} \( \hat{p}\ket{p} = p \ket{p} \Leftrightarrow \braket{p|\hat{p}|\psi}  = p \psi(p) \) con \( \psi(p) = \braket{p|\psi} \), ma allora
\[
