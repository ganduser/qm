...
\[
N \ket{z} = \lambda_z \ket{z} \; \implies \; \begin{cases}
...
\bigskip
\noindent
Inoltre, \( N \ket{z} = \lambda_z \ket{z} \implies \lambda_z \geq 0 \).
...
\end{align*}
Deve esistere \( \ket{0} \) tale che \( \hat{a} \ket{0} = 0 \), perché se non esistesse, cosiderando
\[
N \hat{a}^P \ket{z} = (\lambda_z - P) \hat{a}^P \ket{z},
\]
per \( P \) abbastanza grande avrei autovalori negativi. Inoltre, se \( \hat{a} \ket{0} = 0 \), allora \( N \ket{0} = \hat{a}^\dag \hat{a} \ket{0} = \lambda_0 \ket{0} \), cioè \( \lambda_0 = 0 \).
...
\]
Dico che \( \ket{0} \) è il \textbf{vuoto}, \( N \) è l'\textbf{operatore numero}, \( \hat{a} \) e \( \hat{a}^\dag \) sono gli \textbf{operatori di ditruzione e creazione}.
...
\]
\begin{align*}
\implies \quad 1 &= |K_n|^2 \braket{0|(\hat{a})^n(\hat{a}^\dag)^n|0} = \\
...
\bigskip
\noindent
\textbf{N.B.:} il formalismo di Heisenberg ha permesso di riscrivere tutto senza dover scegliere una base.
...
\]
cioè un caso di equipartizione dell'energia.
...
\]
\[
\implies \quad \Delta^2 \hat{x} \Delta^2 \hat{p} = \hbar^2 \left( n + \frac{1}{2} \right)^2 \geq \frac{\hbar^2}{4}
\]
e \( n = 0 \) dà lo stato a minima indeterminazione.