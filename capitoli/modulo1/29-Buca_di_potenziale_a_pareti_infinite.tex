...
\]
\[
V_0 \to +\infty \implies \frac{d^2}{dx^2} \psi_E(x) = -\frac{2m}{\hbar^2} (E - V_0) \psi_E(x) \to +\infty, \forall E \implies \psi_E(x) = 0.
\]
...
\begin{enumerate}
\item \( B_E = 0 \implies \psi_E^I(x) = A_E \sin(kx) \) quindi per \( x = \pm a \) vale
\begin{align*}
\sin(ka) = -\sin(-ka) = 0 &\implies ka = \pi n \implies k_n = \frac{\pi}{a} n \implies \\
&\implies E_n = \frac{\hbar^2 \pi^2}{2m} \frac{n^2}{a^2}
\end{align*}
\item \( A_E = 0 \implies \psi_E^{II}(x) = B_E \cos(kx) \) e per \( x = \pm a \) ho
\begin{align*}
\cos(ka) = 0 \implies ka = (2n + 1) \frac{\pi}{2} &\implies k_n = \frac{\pi}{2a}(2n + 1) \implies \\ 
&\implies E_n = \frac{\hbar^2 \pi^2}{2m} \frac{(2n + 1)^2}{4a^2}
\end{align*}
\end{enumerate}
...
\[
\lambda_n = \frac{2\pi}{k_n} = \frac{2L}{n} \quad \implies \quad \frac{\lambda_n}{2} = \frac{L}{n}
\]
...
\begin{tikzpicture}
  ...
  \draw[->] (0,-3.5) -- (0,3.5);
  \node (n1) at (-2,2) {\(n = 1\)};
  \draw[dotted,very thick] plot[smooth] file {./grafici/buca_di_potenziale_cos.txt};
  \node (n2) at (2,2.75) {\(n = 2\)};
  \draw[dashed,very thick] plot[smooth] file {./grafici/buca_di_potenziale_sin.txt};

  \node (-a) at (-3.25,-.3) {\(-a\)};
  \node (a) at (3,-.3) {\(a\)};
\end{tikzpicture}
\caption{Andamento dello stato fondamentale (punteggiato) e del primo stato eccitato (tratteggiato) per la buca di potenziale a pareti infinite.}
\label{buca_infinita}
\end{figure}
Pensando alle soluzioni come azione dell'operatore parità sulla funzione d'onda si ha
\[
\braket{x|\mathcal{P}|\psi} = \braket{-x|\psi} \; \implies \; \mathcal{P} \ket{\psi_E} = \begin{cases}
...
\[
\begin{cases}
\Delta x \leq L \implies \Delta^2 x \leq L^2 \\
\Delta^2 \hat{p} \Delta^2 \hat{x} \geq \frac{\hbar^2}{4}
\end{cases} \; \implies \; \Delta^2 \hat{p} \geq \frac{\hbar^2}{4L^2}
\]
