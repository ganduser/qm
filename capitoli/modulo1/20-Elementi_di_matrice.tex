...
\]
Poinché \( \braket{q^\prime|q} = \delta(q - q^\prime) = \delta(q^\prime - q) \), esprimo
\[
\ket{q} \equiv \ket{\psi_q} \quad \implies \quad \braket{q^\prime|\psi_q} = \delta(q - q^\prime) = \psi_q(q^\prime)
\]
da cui
\[
...
\]
ma \( \braket{\varphi|\hat{p}|\psi} = (\braket{\psi|\hat{p}|\varphi})^* \), cioè
\[
...
\]
che integrando per parti dà
\[
i \hbar \int_{-\infty}^{+\infty} \varphi^*(q) \, \psi(q) \, dq = 0
\]
allora devo imporre che \( f \rightarrow 0 \) e \( g \rightarrow 0 \), cioè in particolare devo
\[
