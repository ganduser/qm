...
\textbf{Metodo di Heisenberg}: avr\`o alcune propriet\`a sugli \( E_n \).
\begin{enumerate}
...
\]
quindi \( [\hat{p}, \hat{x}] = 0 \), che \`e falso. Quindi \( \boxed{E_n > 0} \).
...
= 0 \text{ sulla diagonale e anche dove la differenza \`e da } (\hbar \omega) \\
