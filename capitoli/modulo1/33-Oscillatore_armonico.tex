...
\begin{enumerate}
\item \( \left( \frac{\hat{p}^2}{2m} + \frac{1}{2} m \omega^2 \hat{x}^2 \right) \ket{n} = E_n \ket{n}  \implies \braket{n|\left( \frac{\hat{p}^2}{2m} + \frac{1}{2} m \omega^2 \hat{x}^2 \right)|n} = E_n \braket{n|n} \). 
\[
...
\]
Ma se per assurdo \( E_n = 0 \implies \braket{n|\hat{p}|k} = \braket{n|\hat{x}|k} = 0, \forall \ket{k} \), quindi
\[
...
\[
\braket{n|\hat{x}|k} = \frac{i\hbar}{m(E_k - E_n)} \frac{(-i\hbar m \omega^2)}{E_k - E_n} \braket{n|\hat{x}|k} \; \implies
\]
\[
\implies \; (E_k - E_n)^2 \braket{n|\hat{x}|k} = \hbar^2\omega^2 \braket{n|\hat{x}|k}
\]
cioè l'elemento
\[
