...
\]
Allora le probabilità (fase globale)
\[
...
\bigskip
\noindent
Parto dalla considerazione che se non posso ricostruire il sistema con le sue condizioni iniziali,  non ho informazioni sul sistema. Devo poter riprodurre più volte le codizioni iniziali.
...
P_1 = |a_1|^2 \\
P_2 = |a_2|^2
\end{cases} \qquad \implies \qquad P_1, P_2 \text{ deterministiche}
\]
Scrivo
\[
...
\end{cases} \qquad \implies \qquad P_1, P_2 \text{ deterministiche}
\]
Quindi due variabili compatibili danno la stessa informazione. Dovrò prenderne due incompatibili.

\bigskip
\noindent
Introduco l'operatore densità
\[
...
e ricordo che
\begin{enumerate}
\item è invariante per cambio di base,
...
\]
In particolare, l'operatore densità è utile quando di un sistema conosco solo le probabilità associate agli stati.
\[
...
\]
inoltre, poiché \(\braket{A} = \Tr(A\rho)\), vale
\begin{align*}
...
&= \sum_i p_i \braket{\psi_i|A|\psi_i} \qquad \implies
\end{align*}
\[
\implies \qquad \braket{A} = \sum_i p_i \braket{\psi_i|A|\psi_i}
\]
In generale, l'operatore densità associato allo stato puro  è diverso da quello associato allo stato misto: nel primo sopravvivono i termini di interferenza, nel secondo no.

\subsubsection{Esempio: matrice densità e stati}
\[
...
0 & 1 \\
1 & 0
\end{pmatrix} \quad \implies \quad A \ket{\psi_1} = \ket{\psi_1} \quad \implies \quad \braket{A}_{\psi_1} = \braket{\psi_1|A|\psi_1}
\]
...
\end{cases}
\]
allora per uno stato puro \(p_{i_0} = 1\) e \(p_i = 0\) per ogni \(i \ne i_0\) e poiché vale \(\det \rho = \prod_i p_i\),  nel caso bipartito ho
\[
\det \rho \begin{cases}
= 0 \qquad \text{puro} \\
\ne 0 \qquad \text{misto}
...
\subsubsection{Esempio: ancora matrice densità}
\[
...
\]
\[
\Tr(\rho_\psi) = |N|^2 \left[ \left( 1 + \sqrt{2} \right) + 1 \right] = 1 \quad \implies \quad |N|^2 = \frac{1}{2(2 + \sqrt{2})}
\]
\[
\det \rho_\psi = 0
\]
...
\]
\[
\mathrm{misura} \implies \begin{cases}
