...
\[
\mathbbm{1} + i \varepsilon \hat{\mathcal{H}}(t_0) = \mathbbm{1} + i \varepsilon \hat{\mathcal{H}}(t_0 + \delta) \quad \implies \quad \frac{d}{dt} \hat{\mathcal{H}}(t) = 0
\]
cioè \( \hat{\mathcal{H}} \equiv \hat{\mathcal{H}}(\hat{p}, \hat{q}) \) (\textbf{N.B.:} è un ``se e solo se'').
...
&= S(t_1, t_0) + i \delta \left[ \hat{\mathcal{H}}, S(t_1, t_0) \right] + \mathcal{O}(\delta) \quad \implies
\end{align*}
\[
\implies \quad \left[ \hat{\mathcal{H}}, S(t_1, t_0) \right] = 0.
\]
...
\end{align*}
cioè \( \braket{E|S|E^\prime} = 0 \) se \( E \neq E^\prime \).
...
\bigskip
\noindent
Voglio che \(\hat{\mathcal{H}} = -c \mathcal{H}\). Dimensionalmente, poiché
\[
S(t, t_0) \ket{\psi(t_0)} = \ket{\psi(t)} \; \xrightarrow[\text{infinitesim.}] \; \ket{\psi(t)} = e^{(t - t_0) \frac{d}{dt}} \ket{\psi(t)} |_{t_0} =
\]
\[
= \left( \mathbbm{1} + (t - t_0) \frac{d}{dt} + \mathcal{O}(t - t_0) \right) \ket{\psi(t_0)} \quad \implies
\]
\[
\implies \quad \hat{\mathcal{H}}(t) \ket{\psi(t)} = \frac{d}{dt} \ket{\psi(t)}
\]
la costante è un'azione, quindi \( c \equiv \frac{1}{\hbar} \). Riscrivo
\[
\hat{\mathcal{H}} = - \frac{1}{\hbar} \mathcal{H} \left( \hat{p}, \hat{q} \right) \quad \implies \quad \boxed{i \hbar \frac{d}{dt} \ket{\psi(t)} = \mathcal{H}( t; \hat{p}, \hat{q}) \ket{\psi(t)}} \; \text{eq. di Schr\"odinger.}
\]
Per \( \mathcal{H} = T + V = \frac{\hat{p}^2}{2m} + V(\hat{q}) \) per stati \( \ket{q} \)
\[
\braket{q|i \hbar \frac{d}{dt}|\psi(t)} = \braket{q|\frac{\hat{p}^2}{2m} + V(\hat{q})|\psi(t)} \quad \implies
\]
\[
\left[
\begin{aligned}
...
\]
\[
\implies \quad i \hbar \frac{d \psi}{dt}(q; t) = - \frac{\hbar^2}{2m} \frac{d^2 \psi}{dq^2}(q;t) + V(q) \psi(q;t).
\]
...
\]
\[
i \hbar \frac{d}{dt} S(t, t_0) = \mathcal{H}(t) S(t, t_0) \quad \implies \quad \boxed{S(t, t_0) = \exp{\left\{ \frac{1}{i \hbar} \int_{t_0}^{t} \mathcal{H}(t^\prime) \, dt^\prime \right\}}}
\]
Caso 3.
\[
...
\end{align*}
Quindi per l'\(n\)-esimo termine dello sviluppo in serie di Taylor dell'esponenziale applico lo stesso metodo, cioè
\[
...
\]
e gli \( n! \) si semplificano. Adesso controllo se \( S(t, t_0) \) è soluzione dell'equazione di Schr\"odinger.
\begin{align*}
...
&= \mathcal{H}(t) S(t, t_0)
\end{align*}
Nel caso più semplice in cui \( \frac{d}{dt} \mathcal{H} = 0 \) so che
\[
S(t, t_0) = \exp\left\{ \frac{1}{i \hbar} (t - t_0) \mathcal{H} \right\}
\]
allora l'operatore di evoluzione temporale lo posso calcolare solo quando \( \mathcal{H} \ket{n} = E_n \ket{n} \) (diagonalizzabilità dell'operatore), da cui poi \( \braket{n|\psi} = c_n \) e \( p_n = |c_n|^2\).
...
\]
cioè \( \ket{\psi(t_0)} \) e \( \ket{\psi(t)} \) \textbf{differiscono per una fase}. Inoltre
\[
\ket{\psi(t_0)} = \sum_k \ket{k} \overbrace{\braket{k|\psi(t_0)}}^{c_k(t_0)} = \sum_k c_k(t_0) \ket{k} \quad \implies
\]
\[
\implies \quad \ket{\psi(t)} = \sum_n \exp{\left\{ \frac{1}{i \hbar} (t - t_0) E_n \right\}} \ket{n} \overbrace{\braket{n|\psi(t)}}^{c_n(t)} \; \implies
\]
\[
\implies \; c_n(t) = c_n(t_0) \exp{\left\{ \frac{1}{i \hbar} (t - t_0) E_n \right\}}
\]
cioè la probabilità si conserva perché 
\[
