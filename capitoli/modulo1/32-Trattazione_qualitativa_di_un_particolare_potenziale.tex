...
\bigskip
\noindent
Nel caso \( V(x) > E \) sempre, ho soluzioni oscillanti \( \implies \) spettro continuo, cioè c'è almeno uno stato legato.
...
\[
[A, \hat{\mathcal{H}}] = 0 \quad \implies \quad A \ket{E} = a_E \ket{E}
\]
\end{enumerate}
...
\[
\psi_2 \psi_1'' - \psi_1 \psi_2'' = 0 = \frac{d}{dx} (\psi_2 \psi_1' - \psi_1 \psi_2') \; \implies \; \psi_2 \psi_1' - \psi_1 \psi_2' = \text{ costante}
\]
...
\[
\frac{\psi_1'}{\psi_1} = \frac{\psi_2'}{\psi_2} \; \implies \; \frac{d}{dx} \ln \psi_1(x) = \frac{d}{dx} \ln \psi_2(x) \; \implies \; \ln \psi_1(x) = \ln \psi_2(x) + \text{ cost.} \; \implies
\]
\[
\; \implies \psi_1(x) = e^{\text{cost.}} \psi_2(x)
\]
