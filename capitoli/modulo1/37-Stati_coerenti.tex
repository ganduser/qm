...
Sovrapposizione di stati di oscillatore armonico \( \Rightarrow \) pacchetti d'onda ad indeterminazione minima posizione-impulso che non si allargano perché sono soggetti a potenziale. Definisco questi stati in modo che 
\[
...
\end{align*}
Allora la condizione di autostato è data da 
\[
...
\]
questa è la forma degli \textbf{stati coerenti}.
...
\]
Ai tempi successivi, poiché \( \hat{a}(t) = e^{-i\omega t}\hat{a} \), l'indeterminazione si conserva, mentre i valori medi oscillano.
...
\]
ad esempio, per \( \braket{z|-z} = e^{-2|z|^2} \), cioè ho un decadimento esponenziale di \( \braket{z|-z} \), ma poiché 
\[
